...
// Memorizamos los componentes de motion para evitar re-renders
const MotionUl = memo(motion.ul);
const DropdownItem = memo(
  ({ opt, onSelect }: { opt: Option; onSelect: (value: string) => void }) => (
    <li
  ...
  ({
    ...
    return (
      ...
      >
        ...
        {/*Selector Visible */}
        <div
          className={`border p-3 h-12 rounded-xl cursor-pointer bg-transparent relative ${
            open
              ? "border-[var(--hover-color)]"
              : error
              ? "border-red-500 hover:border-b-red-500 hover:border-b-4"
              : "border-gray-300 hover:border-b-4 hover:border-b-[var(--hover-color)]"
          }`}
          onClick={toggleOpen}
          style={
            {
              "--hover-color": inputHoverColor,
              "--focus-color": inputHoverColor,
            } as React.CSSProperties
          }
        >
          <div className="flex items-center justify-between">
            <span className={`flex items-center h-full justify-start mt-1`}>
        ...
        {isMobile && (isBank || isDocumentType) ? (
          <MobileDropdown
            ...
            dropdownClasses="absolute left-0 w-full border rounded z-50 max-h-60 overflow-y-auto"
            inputHoverColor={inputHoverColor}
            isBgFormulariocolor={isBgFormulariocolor}
            textColorform={textColorform}
          ...
          <AnimatePresence>
            {open && (
              <MotionUl
                ...
                className="absolute left-0 w-full border rounded z-50 max-h-60 overflow-y-auto"
              >
