...
export const SelectFloat: React.FC<SelectFloatProps> = ({
  options,
  value,
  onChange,
  error,
  ...
  return (
    <div
      className="relative "
      ref={ref}
      style={{ color: states.textColorform }}
    >
      {/* Floating Label */}
      <label
        className={`absolute left-1 px-1 py-[2px] transition-all bg-transparent ${
          showFloatingLabel ? "text-xs " : " mt-3 "
      ...
      {/*Selector Visible */}
      <div
        className={`border p-3 h-12  rounded-xl cursor-pointer bg-white relative ${
          open
      ...
      {isMobile && (isBank || isDocumentType) ? (
        <MobileDropdown
          ...
          dropdownClasses="absolute left-0 w-full bg-white border border-blue-400 rounded z-50 max-h-60 overflow-y-auto"
        ...
        <AnimatePresence>
          {open && (
            <motion.ul
              ...
              className="absolute left-0 w-full bg-white border border-blue-400 rounded z-50 max-h-60 overflow-y-auto"
            >
